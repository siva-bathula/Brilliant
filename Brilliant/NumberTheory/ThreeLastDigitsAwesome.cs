...
{
    public class ThreeLastDigitsAwesome : ISolve, IBrilliant
    {
        ...
        void ISolve.Solve()
        {
            List<int> primes = Prime.GeneratePrimesSieveOfSundaram(1000);
            ...
            Console.WriteLine("Number of awesome numbers : {0}", wantedNums.Count);
            int tSum = 0;
            foreach (int i in wantedNums) {
                tSum += i;
                Console.Write(" {0}, ", i);
            }
            Console.WriteLine("Total sum of awesome numbers : {0} ", tSum);
            Console.ReadKey();
        }
    }
}